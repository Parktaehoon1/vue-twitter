<template>
  <!-- notifications -->
  <div class="flex-1 border-r border-gray-100 flex flex-col overflow-y-auto">
    <!-- title -->
    <div class="p-3 font-bold text-lg border-b border-gray-100">알림</div>
    <!-- notifications -->
    <div
      class="flex flex-col hover:bg-gray-50 p-3 space-y-2 border-b border-gray-100 cursor-pointer"
      v-for="notification in 10"
      :key="notification"
    >
      <div class="flex justify-between">
        <img
          src="http://picsum.photos/100"
          class="w-10 h-10 rounded-full hover:opacity-80"
        />
        <i
          class="fas fa-ellipsis-h text-gray-500 hover:bg-blue-50 hover:text-primary p-2 rounded-full w-10 h-10 flex items-center justify-center"
        ></i>
      </div>
      <div><span class="font-bold">박태훈</span> 님의 최근 트윗</div>
      <div class="text-gray-500">
        Lorem ipsum dolor sit amet consectetur adipisicing elit. Natus atque
        debitis quisquam dolorum in commodi omnis dignissimos impedit
        accusantium aspernatur aliquid sit inventore suscipit doloribus, rem ab
        placeat error eligendi.
      </div>
    </div>
  </div>
  <!-- trends -->
  <Trends />
</template>

<script>
import Trends from "@/components/TrendsView.vue";
export default {
  components: {
    Trends,
  },
};
</script>

<style></style>
